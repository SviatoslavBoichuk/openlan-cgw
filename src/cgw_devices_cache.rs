...
impl CGWDevicesCache {
    ...
    pub fn update_device_from_cache_device_firmware(
        &mut self,
        key: &String,
        firmware: &String,
    ) -> bool {
        ...
        if let Some(value) = self.cache.get_mut(key) {
            (*value).set_device_firmware(firmware);
            status = true;
        } else {
            debug!(
                "Failed to update device {} id. Requested item does not exist.",
    ...
    pub fn update_device_from_cache_device_uuid(&mut self, key: &String, uuid: u64) -> bool {
        let status: bool;

        if let Some(value) = self.cache.get_mut(key) {
            (*value).set_device_uuid(uuid);
            status = true;
        } else {
            debug!(
                "Failed to update device {} id. Requested item does not exist.",
